...
class PilePredictor(nn.Module):
    """PilePredictor: tokens (+ optional conditioning) + gate -> pile mass w + position p."""
    def __init__(self, dim=384, hidden=128, cond_dim=0):
        super().__init__()
        in_dim = dim + cond_dim
    ...
    def forward(self, tokens, gate, centers, cond=None):
        feats = tokens if cond is None else torch.cat([tokens, cond], dim=-1)
        w = F.softplus(self.mlp_w(feats).squeeze(-1)) * gate.squeeze(-1)  # [B,M]
