"""N0011_dino_pertok_gate_huber — per-token spatial gating + Huber loss for outlier robustness."""
import math
...
class PerTokenGateMLP(nn.Module):
    """Shared MLP: concat(z6, z11) per-token -> softmax gate in R^2."""
    def __init__(self, in_dim=768, hidden=64, out_dim=2):
        super().__init__()
    ...
    def forward(self, z6, z11):
        x = torch.cat([z6, z11], dim=-1)  # [B, N, 768]
        return F.softmax(self.mlp(x), dim=-1)  # [B, N, 2]
...
class DinoPromptV2(nn.Module):
    def __init__(self, cfg):
        ...
        gate_hidden = int(cfg.get("gate_mlp_hidden", 64))
        self.gate_mlp = PerTokenGateMLP(in_dim=ch * 2, hidden=gate_hidden, out_dim=2)
    ...
    def forward(self, imgs, bboxes):
        ...
        z6 = self.t6_proj(f6.flatten(2).transpose(1, 2))   # [B, N, 384]
        z11 = self.t11_proj(f11.flatten(2).transpose(1, 2))  # [B, N, 384]
        gate = self.gate_mlp(z6, z11)                       # [B, N, 2]
        tokens = gate[..., 0:1] * z6 + gate[..., 1:2] * z11  # [B, N, 384]
