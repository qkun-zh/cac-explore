...
N0009_h0015 (H0015): BMNet+-style dynamic per-exemplar channel gate
(`use_dyn_exemplar_gate`, arXiv:2203.08354). A small shared MLP maps EACH exemplar
embedding [B, K, d_model] to its own per-channel weight vector (d_model -> 64 ->
d_model, 2*sigmoid) that is multiplied elementwise into that exemplar's token
before it enters the condenser cross-attention. The last layer is zero-initialized,
so at step 0 the gate is exactly 1.0 and the switch-ON forward is functionally the
champion; with `use_dyn_exemplar_gate` absent the module is not built and the
forward is the champion's exactly.
"""
import torch
...
class DynExemplarGate(nn.Module):
    """H0015: BMNet+-style per-exemplar channel gate.

    One shared MLP maps each exemplar token [B, K, d_model] to its own per-channel
    weight vector; nn.Linear acts on the last dim, so it is applied token-wise and
    K stays free (1..3, no K-dependence). The last layer is zero-initialized and the
    gate is 2*sigmoid(.), which equals 1.0 at init: switch-ON starts at the
    champion's function (no 0.5 channel halving) while the bounded (0, 2) mask still
    lets AdamW carve a per-exemplar channel subspace. The gate multiplies the e
    passed to the condenser, whose attention keys/values are un-normed, so the
    per-exemplar scaling is not cancelled before matching.
    """
    ...
    def forward(self, e):
        return 2.0 * torch.sigmoid(self.mlp(e))
...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.use_dyn_exemplar_gate = _get(cfg, "use_dyn_exemplar_gate", False)
        if self.use_dyn_exemplar_gate:
            self.dyn_exemplar_gate = DynExemplarGate(_get(cfg, "embed_dim", 256), hidden=64)
