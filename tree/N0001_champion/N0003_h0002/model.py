...
class GCA(nn.Module):
    ...
    def __init__(self, D, d_model, use_gca_cal=False):
        super().__init__()
        ...
        self.s_pos = nn.Parameter(torch.tensor(0.02))
        self.s_lin = nn.Parameter(torch.tensor(0.0))
        self.b_cal = nn.Parameter(torch.tensor(0.0))

    def forward(self, fine, e_mean, Hf, Wf):
        gap = fine.mean(dim=(2, 3))
        z = self.gca(torch.cat([gap, e_mean], 1)).squeeze(1)
        n_aux = F.softplus(z)
        if not self.use_gca_cal:
            bias = (n_aux / float(Hf * Wf)).view(-1, 1, 1, 1) * 0.02
        else:
            n_cal = self.s_pos * n_aux + self.s_lin * z + self.b_cal
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        if self.use_gca:
            self.gca = GCA(D, _get(cfg, "embed_dim", 256), use_gca_cal=_get(cfg, "use_gca_cal", False))
