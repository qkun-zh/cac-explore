...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.use_channel_gate = _get(cfg, "use_channel_gate", False)
        if self.use_channel_gate:
            self.channel_gate = nn.Linear(D, _get(cfg, "embed_dim", 256))
        self.use_const_gate_input = _get(cfg, "use_const_gate_input", False)
        if self.use_channel_gate and self.use_const_gate_input:
            rng = torch.Generator().manual_seed(int(_get(cfg, "seed", 0)))
            fixed = torch.randn(D, generator=rng)
            fixed.requires_grad_(False)
            self.register_buffer("frozen_gate_input", fixed, persistent=False)
        self.use_exemplar_gate = _get(cfg, "use_exemplar_gate", False)
        if self.use_exemplar_gate:
            self.exemplar_gate = nn.Linear(_get(cfg, "embed_dim", 256), 1)
            nn.init.zeros_(self.exemplar_gate.weight)
            nn.init.constant_(self.exemplar_gate.bias, 3.0)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        if self.use_channel_gate:
            if self.use_const_gate_input:
                gate_in = self.frozen_gate_input.expand(B, -1)
            else:
                gate_in = fine.mean(dim=(2, 3))
            gate = torch.sigmoid(self.channel_gate(gate_in))
            e = e * gate.unsqueeze(1)
        if self.use_exemplar_gate:
            e = e * torch.sigmoid(self.exemplar_gate(e))
