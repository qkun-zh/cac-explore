...
N0007_h0013 (H0013): learned per-channel depth-to-space subpixel upsample replaces the
fixed bilinear upsample that brings the coarse backbone readout (top branch) to the fused
resolution (`use_subpixel_up`). A grouped (per-channel) 3x3 conv expands d_fine to 4*d_fine
channels, then PixelShuffle(2) halves channels / doubles spatial size — same resolution as
bilinear, only 5120 new params. Removing `use_subpixel_up` restores the champion fuser.
"""
import torch
...
class FineFuser(nn.Module):
    ...
    Optional DDCA = parallel dilated dw branch (use_ddca). Optional learned per-channel
    depth-to-space subpixel upsample on the coarse readout (use_subpixel_up). Self-contained."""
    def __init__(self, ch_coarse, ch_mid, d_fine=128, use_ddca=True, use_subpixel_up=False):
        super().__init__()
        self.use_ddca = use_ddca
        self.use_subpixel_up = use_subpixel_up
        ...
        if use_subpixel_up:
            self.top_up = nn.Sequential(
                nn.Conv2d(d_fine, 4 * d_fine, 3, padding=1, groups=d_fine),
                nn.PixelShuffle(2))

    def forward(self, h2, h3):
        top = self.top(h3)
        if self.use_subpixel_up:
            top = self.top_up(top)
        else:
            top = F.interpolate(top, scale_factor=2, mode="bilinear", align_corners=False)
