...
N0006_h0012 (H0012): count-conditioned condenser cross-attention temperature
(`use_count_temp`). A small MLP reads GAP(fine) + mean exemplar embedding and emits a
global count prior z; the condenser temperature tau = clamp(1+z, 0.25, 4.0) scales the
cross-attention logits (q,k pre-scaled by 1/sqrt(tau)). Removing `use_count_temp` from
config restores the champion condenser exactly.
"""
import torch
...
class Condenser(nn.Module):
    def __init__(self, d_in=128, d_sim=256, n_heads=4, ff=512, d_out=64, use_count_temp=False):
        super().__init__()
        self.use_count_temp = use_count_temp
    ...
    def forward(self, tok, e, tau=None):
        tok = self.proj_in(tok)
        q = self.norm1(tok)
        if tau is not None and self.use_count_temp:
            t = tau.clamp_min(1e-3).unsqueeze(1).sqrt()
            a, _ = self.attn(q / t, e / t, e, need_weights=False)
        else:
            a, _ = self.attn(q, e, e, need_weights=False)
        q = self.norm1(tok + a)
        return self.out(self.norm2(q + self.ffn(q)))


class CountPrior(nn.Module):
    """Global count prior MLP: maps GAP(fine) + mean exemplar embedding to a scalar logit.
    Zero-initialized so tau = clamp(1 + z, 0.25, 4.0) starts at exactly 1 (= champion)."""
    ...
    def forward(self, gap, e_mean):
        return self.mlp(torch.cat([gap, e_mean], 1))
...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.cond = Condenser(d_in=D, d_sim=_get(cfg, "embed_dim", 256), d_out=_get(cfg, "cond_dim", 64),
                              use_count_temp=_get(cfg, "use_count_temp", False))
        self.decoder = DensityDecoder(in_ch=D + _get(cfg, "cond_dim", 64), hidden=2 * D)
        self.S = _get(cfg, "input_size", 384)
        self.use_count_temp = _get(cfg, "use_count_temp", False)
        if self.use_count_temp:
            self.count_prior = CountPrior(D, _get(cfg, "embed_dim", 256))
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        e_mean = e.mean(dim=1)
        if self.use_count_temp:
            z = self.count_prior(fine.mean(dim=(2, 3)), e_mean)
            tau = torch.clamp(1.0 + z, 0.25, 4.0)
        else:
            tau = None
        cond = self.cond(fmap, e, tau)
