...
N0010_h0016 (H0016): DAVE-lite verify-and-suppress (`use_verify_mask`, arXiv:2404.16622,
jerpelhan/DAVE). A VerifyMask module — constructed LAST in CountingHead.__init__ with
zero-RNG explicit-zero scalars, so every shared-module init draw stays the champion's —
takes the preliminary decoder density, selects its top-K peaks, pools the fused fine
feature there and at the exemplar boxes (same fine-feature space), scores each peak by
cosine similarity against the exemplar prototype, and multiplies the density at those
peaks by the learned-threshold gate `1 - a*sigmoid(b - cos)` (a = gain, zero-init;
b = learned offset on the similarity, so low-similarity peaks are suppressed while
verified peaks keep gate ~ 1). At init a = 0, so the gate is exactly 1 and
switch-ON is bit-identically the champion at step 0; with `use_verify_mask` absent the
module is not built and the forward is exactly the champion's.
"""
import torch
...
class VerifyMask(nn.Module):
    """H0016: DAVE-lite verify-and-suppress (arXiv:2404.16622).

    Inputs (shapes derived from the tensors, any input size): preliminary density
    `dens` [B, 1, Hf, Wf], fused fine feature `fine` [B, D, Hf, Wf], exemplar boxes
    `bboxes_in` [B, Ke, 4] in input-image coordinates.

    Per image the K = max(8, round(0.01*Hf*Wf)) highest-scoring density peaks are
    selected — indexing only, no gradient through the selection. `fine` is gathered
    at those peaks and roi_align-pooled (1x1) at the exemplar boxes; both are in the
    SAME fine-feature space, so each peak is scored by plain cosine similarity
    `cos` in [-1, 1] against the exemplar prototype. The density is then multiplied
    at those peaks by the learned-threshold gate

        gate = clamp(1 - a * sigmoid(b - cos), 0, 1 + 1e-6)

    with a = `raw_gain` (zero-init) and b = `b` (learned offset / threshold on the
    similarity, gate's half-point sits at cos ~ b). The sign of (b - cos) makes the
    gate monotone non-decreasing in cos: peaks whose appearance agrees with the
    exemplar prototype (cos > b) keep gate ~ 1, disagreeing peaks (cos < b) are
    suppressed toward 1 - a.

    Init math: a = 0 -> gate = 1 - 0*sigmoid(.) = 1 exactly for every cos, so the
    scattered suppression 1 - gate is 0 and `dens` comes back bit-identically (the
    model starts as the champion). The max headroom (1 + 1e-6, not 1) keeps the
    clamp strictly inactive at init, so dL/da = -sum_peaks dL/dgate * sigmoid(b-cos)
    != 0 on the first step (sigmoid > 0) under any boundary convention, and caps any
    learned enhancement at 1e-6 (suppression-only in practice). b receives gradient
    once a != 0 (step >= 1); `fine` gets a gradient route through the pooled peak and
    prototype features once a != 0. Params: exactly 2 scalars; forwards cost is
    O(B*N*log K + B*K*D) with K ~ 1% of N, negligible beside the frozen backbone.
    """
    def __init__(self, img_size=384):
        super().__init__()
        self.S = float(img_size)
        self.raw_gain = nn.Parameter(torch.zeros(()))
        self.b = nn.Parameter(torch.zeros(()))

    def forward(self, dens, fine, bboxes_in):
        B, _, Hf, Wf = dens.shape
        D = fine.shape[1]
        N = Hf * Wf
        K = min(max(8, int(round(0.01 * N))), N)
        idx = dens.detach().reshape(B, N).topk(K, dim=1).indices
        peak = torch.gather(fine.reshape(B, D, N), 2,
                            idx.unsqueeze(1).expand(B, D, K)).transpose(1, 2)
        Ke = bboxes_in.shape[1]
        s = Wf / self.S
        bidx = torch.arange(B, device=bboxes_in.device, dtype=bboxes_in.dtype).view(B, 1, 1).expand(B, Ke, 1)
        rois = torch.cat([bidx, bboxes_in * s], -1).reshape(B * Ke, 5).to(fine.dtype)
        proto = roi_align(fine, rois, output_size=(1, 1)).reshape(B, Ke, D).mean(1)
        peak = F.normalize(peak.float(), dim=-1)
        proto = F.normalize(proto.float(), dim=-1)
        cos = (peak * proto.unsqueeze(1)).sum(-1)
        gate = 1.0 - self.raw_gain * torch.sigmoid(self.b - cos)
        gate = torch.clamp(gate, min=0.0, max=1.0 + 1e-6).to(dens.dtype)
        supp = dens.new_zeros(B, N).scatter(1, idx, 1.0 - gate)
        return dens * (1.0 - supp.reshape(B, 1, Hf, Wf))
...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.use_verify_mask = _get(cfg, "use_verify_mask", False)
        if self.use_verify_mask:
            self.verify_mask = VerifyMask(self.S)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        if self.use_verify_mask:
            dens = self.verify_mask(dens, fine, bboxes_in)
        e_mean = e.mean(dim=1)
