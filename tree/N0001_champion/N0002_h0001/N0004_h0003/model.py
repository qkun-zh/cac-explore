...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0003 switch (non-module flag: no RNG consumption). Module attached later
        # in Counter.__init__ (after SimPrior) to keep append-only RNG order.
        self.use_padapt = _get(cfg, "use_padapt", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        # H0003 (use_padapt): image-conditioned adaptation of the Condenser K/V only.
        # Flag off -> e_cond is e and forward is the parent's byte-for-byte.
        e_cond = e
        if self.use_padapt:
            e_cond = self.padapt(e, h2)
        cond = self.cond(fmap, e_cond)
        ...
        # H0003: SimPrior reads the ORIGINAL e (single-switch discipline).
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0003: PrototypeAdapter is constructed AFTER every parent module and
        # after SimPrior (append-only RNG order, AGENTS rule 13) and attached to
        # the head. All parent init draws keep their exact RNG positions; only
        # PrototypeAdapter's own init draws are appended. Forward gating lives in
        # CountingHead.forward via self.use_padapt.
        self.use_padapt = _get(cfg, "use_padapt", False)
