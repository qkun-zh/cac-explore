...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0007 switch (non-module flag: no RNG consumption, mirrors use_simprior style).
        # NOTE: idea.md writes `_get(cfg, "use_simcal", False)` inside forward, but cfg is
        # not in forward scope — this flag mirrors the use_simprior pattern instead, so
        # flag-off forward is the parent's byte-for-byte.
        self.use_simcal = _get(cfg, "use_simcal", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        # H0007 (use_simcal): statistics-only calibration of the reused simprior volume.
        # Flag off -> skipped and forward is the parent's byte-for-byte.
        if self.use_simcal:
            # S reused read-only from simprior projections (recompute path; no new
            # qproj/kproj, SimPrior weights/shapes untouched); SimCal adds statistics,
            # not matchers.
            q = F.normalize(self.simprior.qproj(fine), dim=1)
            p = F.normalize(self.simprior.kproj(e), dim=-1)
            S = torch.einsum('bchw,bkc->bkhw', q, p)
...
class SimCal(nn.Module):
    """H0007 similarity-distribution calibration (`use_simcal`).

    Statistics-only readout over the REUSED simprior cosine volume S (read-only;
    no new qproj/kproj, SimPrior untouched): per-exemplar spatial de-mean +
    SNR re-weight, magnitude-weighted consensus + calibrated margin, compressed
    through a zero-init 1x1 projection added residually into `cond_map`.
    Pure additive residual at the decoder interface: decoder in_ch stays 192,
    no parent module shape/init touched, `e` and `fine` read-only (never
    gated/rescaled). Zero-init `out` + tau=1.0 makes step-0 delta exactly 0.
    Forward has no stochastic ops and no dropout.
    """
    ...
    def forward(self, S):
        # S: (B,K=3,96,96) cosine volume recomputed read-only via simprior projections
        ...
        w = (snr * self.tau).softmax(dim=1).unsqueeze(-1).unsqueeze(-1)  # (B,K,1,1)
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0007: SimCal constructed AFTER every parent module (append-only RNG order,
        # AGENTS rule 13) and attached to the head; last nn.Module in __init__ so all
        # parent init draws keep exact RNG positions. Own init is zeroed out + scalar.
        self.head.simcal = SimCal(cond_dim=_get(cfg, "cond_dim", 64))
