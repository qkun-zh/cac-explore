...
N0006_h0005 (H0005 use_verify): adds zero-init repetition-gated
proposal-verification residual (Verify) into cond_map. Single-switch delta from
parent N0002_h0001; flag off -> parent forward byte-for-byte.
"""
import torch
...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0005 switch (non-module flag: no RNG consumption, safe before Verify wiring).
        self.use_verify = _get(cfg, "use_verify", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        # H0005 (use_verify): zero-init repetition-gated proposal-verification
        # residual into cond_map. Flag off -> skipped, forward is the parent's
        # byte-for-byte. Decoder in_ch unchanged (192).
        if self.use_verify:
            cond_map = cond_map + self.verify(fine, e, cond_map)
...
class Verify(nn.Module):
    ...
    Per-cell P×V gate: a dense proposal head P from `cond_map` multiplied by
    a sigmoid verification mask V conditioned on the projected fine query q
    plus 2ch per-cell exemplar-repetition evidence [top1, consensus].
    Additive residual into post-Condenser `cond_map` only: decoder in_ch
    stays 192, `e`/`fine` are read-only (never gated/rescaled), no parent
    module shape/init is touched. Zero-init `prop2` makes step-0 residual
    all-zero (forward numerically identical to the parent); `ver2` bias 0
    gives V = sigmoid(0) = 0.5 at step 0.
    """
    def __init__(self, d_fine=128, d_model=256, d_q=32, cond_dim=64, d_h=32, n_ev=2):
        ...
        nn.init.zeros_(self.prop2.weight)
        nn.init.zeros_(self.prop2.bias)
        nn.init.zeros_(self.ver2.weight)
        nn.init.zeros_(self.ver2.bias)  # V = sigmoid(0) = 0.5 at step 0

    def forward(self, fine, e, cond_map):
        # fine: (B,128,96,96); e: (B,K=3,256); cond_map: (B,64,96,96)
        ...
        return P * V                                        # (B,64,96,96), all-zero at init via prop2
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0005: Verify is constructed AFTER all parent modules (append-only
        # RNG order, AGENTS rule 13) so every shared-module init draw stays
        # identical to the parent's; only Verify's own init draws are
        # appended. Forward gating lives in CountingHead.forward via
        # self.use_verify. Decoder in_ch untouched (192).
        self.head.verify = Verify(d_fine=D, d_model=_get(cfg, "embed_dim", 256),
                                  cond_dim=_get(cfg, "cond_dim", 64))
