...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0013 switch (non-module flag: no RNG consumption).
        self.use_negsup = _get(cfg, "use_negsup", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        if self.use_simprior:
            # H0013 (use_negsup): two-call probe/adjust form nested inside the
            # use_simprior branch. Flag off -> parent-exact single call below.
            if self.use_negsup:
                if self.use_cellcal:
                    with torch.no_grad():
                        _, pos_d = self.simprior(fine, e, use_cellcal=True, w_c=self.cellcal.w_c, return_top1=True)
                    adj = self.negsup.adjustment(fine.detach(), h3.detach(), bboxes_in, pos_d)
                    cond_map = cond_map + self.simprior(fine, e, use_cellcal=True, w_c=self.cellcal.w_c, ev_adj=adj)
                else:
                    with torch.no_grad():
                        _, pos_d = self.simprior(fine, e, return_top1=True)
                    adj = self.negsup.adjustment(fine.detach(), h3.detach(), bboxes_in, pos_d)
                    cond_map = cond_map + self.simprior(fine, e, ev_adj=adj)
            elif self.use_cellcal:
                cond_map = cond_map + self.simprior(fine, e, use_cellcal=True, w_c=self.cellcal.w_c)
            else:
...
class SimPrior(nn.Module):
    ...
    def forward(self, fine, e, use_cellcal=False, w_c=None, return_top1=False, ev_adj=None):
        ...
        if ev_adj is not None:
            ev = ev - ev_adj                                    # H0013: background suppression (broadcast over ev)
        out = self.out(ev)                                      # (B,64,96,96), all-zeros at init
        if return_top1:
            return out, top1.detach()
        return out
...
class NegSup(nn.Module):
    """H0013 background-prototype suppression (`use_negsup`).

    Own projections (Wqn/Wbn, never shared with SimPrior) + ONE zero-init
    scalar `lam`. Builds J=4 deterministic background prototypes per image
    from frozen h3 cells outside the exemplar boxes (each expanded 8px
    S-space), matches them densely against fine in the own 32-d space, and
    returns `lam * ReLU(neg - pos - 0.1)` subtracted from post-cellcal-gain
    `ev` inside SimPrior. `lam`=0 is the identity (byte-identical to the
    parent at init). Holds NO normalization layers, so the no_grad probe
    call is side-effect-free in train and eval.
    """
    def __init__(self, d_fine=128, h3_dim=384, d_neg=32, j_slots=4, s_size=384):
        super().__init__()
        self.Wqn = nn.Conv2d(d_fine, d_neg, 1, bias=False)  # own query projection
        self.Wbn = nn.Linear(h3_dim, d_neg, bias=False)     # own background-bank projection
        self.lam = nn.Parameter(torch.zeros(()))            # suppression strength, ZERO-INIT
        self.j_slots = j_slots
        self.s_size = s_size

    def adjustment(self, fine_d, h3_d, bboxes, pos_d):
        # fine_d: (B,D,96,96) detached; h3_d: (B,C3,H3,W3) detached;
        # bboxes: (B,K,4) S-space x1y1x2y2 (the tensor the exemplar path got);
        # pos_d: (B,1,96,96) detached SimPrior top1. Returns (B,1,96,96).
        J = self.j_slots
        S = float(self.s_size)
        H3, W3 = h3_d.shape[2], h3_d.shape[3]
        C3 = h3_d.shape[1]
        with torch.no_grad():
            bb = bboxes.detach()
            if bb.dim() == 2:
                bb = bb.unsqueeze(1)
            K = bb.shape[1]
            bb_e = torch.stack([                      # expand 8px S-space, clamp
                (bb[..., 0] - 8.0).clamp_min(0.0),
                (bb[..., 1] - 8.0).clamp_min(0.0),
                (bb[..., 2] + 8.0).clamp_max(S),
                (bb[..., 3] + 8.0).clamp_max(S),
            ], dim=-1)                                # (B,K,4)
            sx = S / float(W3)                        # S-space px per h3 cell (16 at S=384)
            sy = S / float(H3)
            rows = []
            for b in range(bb_e.shape[0]):
                covered = torch.zeros((H3, W3), dtype=torch.bool, device=h3_d.device)
                for k in range(K):
                    x1 = float(bb_e[b, k, 0]); y1 = float(bb_e[b, k, 1])
                    x2 = float(bb_e[b, k, 2]); y2 = float(bb_e[b, k, 3])
                    ix0 = max(0, math.floor(x1 / sx)); ix1 = min(W3, math.ceil(x2 / sx))
                    iy0 = max(0, math.floor(y1 / sy)); iy1 = min(H3, math.ceil(y2 / sy))
                    if ix1 > ix0 and iy1 > iy0:
                        covered[iy0:iy1, ix0:ix1] = True
                sel = torch.nonzero(~covered, as_tuple=False)  # raster order, (M,2) as (iy,ix)
                feat = h3_d.detach()[b]                        # (C3,H3,W3), no backbone grad
                M = sel.shape[0]
                if M == 0:                                     # full-coverage fallback: global mean xJ
                    g = feat.mean(dim=(1, 2))
                    bank_b = g.unsqueeze(0).expand(J, C3)
                else:
                    sz = (M + J - 1) // J                      # ceil(M/J): all M cells covered once
                    protos = []
                    prev = None
                    for j in range(J):
                        chunk = sel[j * sz:(j + 1) * sz]
                        if chunk.shape[0] == 0:                # empty run -> repeat previous
                            protos.append(prev)
                        else:
                            p = feat[:, chunk[:, 0], chunk[:, 1]].mean(dim=1)
                            protos.append(p)
                            prev = p
                    bank_b = torch.stack(protos, dim=0)        # (J,C3)
                rows.append(bank_b)
            bank = torch.stack(rows, dim=0)                    # (B,J,C3), grad-free constants
        qn = F.normalize(self.Wqn(fine_d), dim=1)              # (B,32,96,96)
        bn = F.normalize(self.Wbn(bank), dim=-1)               # (B,J,32)
        Sneg = torch.einsum('bchw,bjc->bjhw', qn, bn)          # (B,J,96,96) cosine in [-1,1]
        neg = Sneg.max(dim=1, keepdim=True).values             # (B,1,96,96) worst-background match
        supp = F.relu(neg - pos_d - 0.1)                       # (B,1,96,96); margin 0.1 FIXED
        return self.lam * supp
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0013: NegSup is constructed LAST, after the CellCal attach
        # (append-only RNG order, AGENTS rule 13). Only Wqn/Wbn Kaiming draws
        # are appended; `lam` zero-init draws no RNG. Forward gating lives in
        # CountingHead.forward via self.use_negsup (nested in use_simprior).
        self.head.negsup = NegSup(d_fine=D, h3_dim=dims[1], s_size=self.S)
