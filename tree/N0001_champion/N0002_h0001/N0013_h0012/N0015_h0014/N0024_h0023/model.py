...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0023 switch (non-module flag: no RNG consumption).
        self.use_antimatch = _get(cfg, "use_antimatch", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        if self.use_simprior:
            am = dict(use_antimatch=True) if self.use_antimatch else {}
            if self.use_cellcal:
                if self.use_peakcal:
                    cond_map = cond_map + self.simprior(fine, e, use_cellcal=True, w_c=self.cellcal.w_c, use_peakcal=True, w_p=self.peakcal.w_p, **am)
                else:
                    cond_map = cond_map + self.simprior(fine, e, use_cellcal=True, w_c=self.cellcal.w_c, **am)
            else:
                if self.use_peakcal:
                    cond_map = cond_map + self.simprior(fine, e, use_peakcal=True, w_p=self.peakcal.w_p, **am)
                else:
                    cond_map = cond_map + self.simprior(fine, e, **am)
...
class SimPrior(nn.Module):
    ...
    def forward(self, fine, e, use_antimatch=False, use_cellcal=False, w_c=None, use_peakcal=False, w_p=None):
        ...
        if use_antimatch:
            m = fine.detach().mean(dim=1, keepdim=True).clamp_min(0)   # (B,1,96,96) same surrogate as cellcal
            mbar = m.mean(dim=(2, 3), keepdim=True)                    # (B,1,1,1)
            mhat = (m / (mbar + 1e-6)).clamp(0, 10)                    # (B,1,96,96) mean-1 positive field, bounded
            anti = (ev[:, :1].mean(dim=(1, 2, 3), keepdim=True) < 0).float().detach()  # (B,1,1,1) hard route, stop-gradient
            en_ev = torch.cat([mhat, mhat], dim=1)                    # (B,2,96,96) in-phase substrate, both channels
            ev = anti * en_ev + (1 - anti) * ev                        # hard swap; route=0 keeps ev identically
        if use_cellcal:
