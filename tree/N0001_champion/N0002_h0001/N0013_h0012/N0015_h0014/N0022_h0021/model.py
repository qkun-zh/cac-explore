...
def _exkern_calibrated(fine, bboxes, S):
    """H0021 non-parametric exemplar-kernel self-calibrated field (`use_exkern`).

    Per image: RoI-pool each exemplar box from `fine.detach()` at its clamped
    fine-cell size, pool to fixed (4,4), average over K -> depthwise kernel,
    convolve over detached fine, channel-mean -> ch, then self-calibrate so
    exemplar regions average 1.0. Returns ch_cal (B,1,Hf,Wf). No learned
    params, no RNG, `e` never touched. Guards: sizes clamped [2,12];
    degenerate (zero-area / non-finite) boxes skipped with a whole-image
    pooled-kernel fallback and a global-mean calibration fallback, so no
    empty-tensor mean ever occurs (H0013 NaN lesson); eps 1e-6 + nan_to_num
    on the division (also keeps w_x=0 step-0 exact: 0 * finite == 0).
    """
    B, C, H, W = fine.shape
    s = W / float(S)
    fd = fine.detach()
    ...
    for b in range(B):
        ...
        for k in range(Kb.shape[0]):
            bk = Kb[k]
            if not bool(torch.isfinite(bk).all()):
                continue
            x1, y1, x2, y2 = bk.tolist()
            if not (x2 > x1 and y2 > y1):
                continue
            rw = int(min(12.0, max(2.0, round((x2 - x1) * s))))
            rh = int(min(12.0, max(2.0, round((y2 - y1) * s))))
            box_f = torch.tensor([[0.0, x1 * s, y1 * s, x2 * s, y2 * s]],
                                 device=fd.device, dtype=fd.dtype)
            roi = roi_align(fb, box_f, output_size=(rh, rw))
            Z.append(F.adaptive_avg_pool2d(roi, (4, 4)))
            valid.append(box_f)
        if not Z:
            # Fallback global-mean kernel: whole-image pooled field.
            Z.append(F.adaptive_avg_pool2d(fb, (4, 4)))
        Kbar = torch.stack(Z, dim=0).mean(dim=0)  # (1,C,4,4)
        weight = Kbar.squeeze(0).unsqueeze(1).detach()  # (C,1,4,4) data, no grad
        match = F.conv2d(fb, weight, bias=None, stride=1, padding=2, groups=C)
        match = match[..., :H, :W]  # k=4/pad=2 yields H+1; crop to fine grid
        ch = match.mean(dim=1, keepdim=True)  # (1,1,H,W)
        chd = ch.detach()
        cs = []
        for box_f in valid:
            cs.append(roi_align(chd, box_f, output_size=(4, 4)).mean())
        if not cs:
            c = chd.mean()
        else:
            c = torch.stack(cs).mean()
        ch_cal = (ch / (c + 1e-6) * 1.0).nan_to_num(0.0, 0.0, 0.0)
        out.append(ch_cal)
...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0021 switch (non-module flag: no RNG consumption).
        self.use_exkern = _get(cfg, "use_exkern", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        if self.use_simprior:
            if self.use_cellcal:
                if self.use_peakcal:
                    cond_map = cond_map + self.simprior(fine, e, use_cellcal=True, w_c=self.cellcal.w_c, use_peakcal=True, w_p=self.peakcal.w_p)
                else:
                    cond_map = cond_map + self.simprior(fine, e, use_cellcal=True, w_c=self.cellcal.w_c)
            else:
                if self.use_peakcal:
                    cond_map = cond_map + self.simprior(fine, e, use_peakcal=True, w_p=self.peakcal.w_p)
                else:
                    cond_map = cond_map + self.simprior(fine, e)
        # H0021 (use_exkern): non-parametric exemplar-kernel self-calibrated
        # residual into cond. Flag off -> skipped, byte-identical to parent.
        if self.use_exkern:
            cond_map = cond_map + self.exkern.w_x * _exkern_calibrated(fine, bboxes_in, self.S)
...
class SimPrior(nn.Module):
    ...
    def forward(self, fine, e, use_cellcal=False, w_c=None, use_peakcal=False, w_p=None):
...
class ExKern(nn.Module):
    """H0021 exemplar-kernel fusion scalar (`use_exkern`).

    Scalar-only pluggable module: ONE zero-init scalar `w_x`. The kernel +
    self-calibration math itself lives in `_exkern_calibrated` (gated on the
    head flag `use_exkern`); this module only owns `w_x` so the head flag
    stays a non-module switch like `use_simprior`. Zero-init draws no RNG, so
    append-only construction keeps parent init draws identical. Kernels are
    data (fine.detach() + boxes); no temperature/softmax/learned norm.
    """
    def __init__(self):
        super().__init__()
        self.w_x = nn.Parameter(torch.zeros(()))
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0021: ExKern scalar is constructed LAST, after the PeakCal attach
        # (append-only RNG order, AGENTS rule 13). Zero-init draws no RNG, so
        # parent init draws keep their exact RNG positions. Forward gating
        # lives in CountingHead.forward via self.use_exkern.
        self.head.exkern = ExKern()
        # Temp-pin hygiene: when use_exkern is on, pin simprior temp
        # (SimPrior.forward W line already detaches; verified intact above).
        if self.head.use_exkern:
            self.head.simprior.temp.requires_grad_(False)
