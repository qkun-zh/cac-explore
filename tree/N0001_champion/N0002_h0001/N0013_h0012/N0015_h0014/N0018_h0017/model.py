...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0017 switch (non-module flag: no RNG consumption).
        self.use_peakmass = _get(cfg, "use_peakmass", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        # H0017 (use_peakmass): peak-gated additive mass placement, pre-decoder.
        # Flag off -> skipped and forward is the parent's byte-for-byte.
        if self.use_peakmass:
            ...
            peak = (mhat / (F.avg_pool2d(mhat, 3, stride=1, padding=1) + 1e-4)).clamp(0, 5)
            peakness = (peak - 1).clamp_min(0)                        # ReLU: zero on flat, >0 on sharp centers
            placed = self.peakmass(mhat * peakness)
            cond_map = cond_map + placed
...
class PeakMass(nn.Module):
    """H0017 peak-gated additive mass placement (`use_peakmass`).

    Single 1x1 projection (1 -> cond_dim), weight AND bias ZERO-INIT so
    step-0 `placed` is all-zeros and forward is the parent's byte-for-byte.
    Zero-init draws no RNG; constructed LAST keeps parent init draws
    identical (AGENTS rule 13). NO norm layers (train/eval agree exactly).
    """
    def __init__(self, cond_dim=64):
        super().__init__()
        self.proj = nn.Conv2d(1, cond_dim, 1)
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0017: PeakMass is constructed LAST, after the PeakCal attach
        # (append-only RNG order, AGENTS rule 13). Zero-init draws no RNG, so
        # parent init draws keep their exact RNG positions. Forward gating
        # lives in CountingHead.forward via self.use_peakmass.
        self.head.peakmass = PeakMass(cond_dim=_get(cfg, "cond_dim", 64))
        # Temp-pin hygiene: when use_peakmass is on, pin simprior temp
        # (PeakMass owns no temperature; no free-temp path reopens).
        if self.head.use_peakmass:
            self.head.simprior.temp.requires_grad_(False)
