...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0025 switch (non-module flag: no RNG consumption).
        self.use_capfilm = _get(cfg, "use_capfilm", False)
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0025: CapFilm constructed AFTER every parent module (append-only RNG
        # order, AGENTS rule 13). First Linear draws default-init RNG, so it must
        # append after PeakCal/simprior/etc — only CapFilm's own draws are added.
        # Last Linear zero-init makes step-0 FiLM an identity.
        if self.head.use_capfilm:
            self.head.capfilm = CapFilm(d_fine=D, d_out=D)
