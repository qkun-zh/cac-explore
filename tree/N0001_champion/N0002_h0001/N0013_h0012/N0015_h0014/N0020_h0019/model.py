...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0019 switch (non-module flag: no RNG consumption).
        self.use_exlanes = _get(cfg, "use_exlanes", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        # H0019 (use_exlanes): additive separate-lane fusion residual into cond.
        # Flag off -> skipped and forward is the parent's byte-for-byte.
        # Pass attached fine/e; the lane detaches its own key copy internally.
        if self.use_exlanes:
            cond_map = cond_map + self.exlanes(fine, e)
...
class ExLanes(nn.Module):
    """H0019 separate per-exemplar similarity lanes with own projections (`use_exlanes`).

    Three lanes, each with FULLY OWN projections (no sharing with SimPrior /
    Condenser / CellCal / PeakCal or across lanes): qproj_k = Conv2d(d_fine->32,
    1x1, bias=False), kproj_k = Linear(d_model->32, bias=False). Lane maps are
    raw cosines stacked (B,3,H,W) and fused by a zero-init 1x1 conv (3->cond_dim).
    No temperature, no softmax-over-K, no per-exemplar scalars, no norms with
    running state. Zero-init `fuse` makes step-0 forward numerically identical
    to the parent. K is fixed at 3 in this protocol.
    """
    def __init__(self, d_fine=128, d_model=256, d_lane=32, cond_dim=64):
        super().__init__()
        self.qprojs = nn.ModuleList([nn.Conv2d(d_fine, d_lane, 1, bias=False) for _ in range(3)])
        self.kprojs = nn.ModuleList([nn.Linear(d_model, d_lane, bias=False) for _ in range(3)])
        self.fuse = nn.Conv2d(3, cond_dim, 1)                 # 3 -> 64, ZERO-INIT
        nn.init.zeros_(self.fuse.weight)
        nn.init.zeros_(self.fuse.bias)

    def forward(self, fine, e):
        # fine: (B,d_fine,H,W) attached (grad flows to fuser via qproj);
        # e: (B,K=3,d_model) attached (this lane detaches its own key copy).
        if e.shape[1] != 3:
            raise ValueError(f"ExLanes expects K=3 exemplars, got K={e.shape[1]}; K is fixed at 3 in this protocol")
        e_det = e.detach()
        lanes = []
        for k in range(3):
            q = F.normalize(self.qprojs[k](fine), dim=1)      # (B,32,H,W)
            p = F.normalize(self.kprojs[k](e_det[:, k, :]), dim=-1)  # (B,32)
            S_k = torch.einsum('bchw,bc->bhw', q, p).unsqueeze(1)    # (B,1,H,W) cosine in [-1,1]
            lanes.append(S_k)
        L = torch.cat(lanes, dim=1)                           # (B,3,H,W)
        return self.fuse(L)                                   # (B,cond_dim,H,W), all-zeros at init
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0019: ExLanes is constructed LAST, after the PeakCal attach
        # (append-only RNG order, AGENTS rule 13). Only ExLanes' own
        # projection draws are appended, so every parent init draw keeps its
        # exact RNG position. Forward gating lives in CountingHead.forward
        # via self.use_exlanes.
        self.head.exlanes = ExLanes(d_fine=D, d_model=_get(cfg, "embed_dim", 256),
                                    cond_dim=_get(cfg, "cond_dim", 64))
        # Temp-pin hygiene: when use_exlanes is on, pin simprior temp
        # (W line already detaches — value-identical; do not duplicate it).
        if self.head.use_exlanes:
            self.head.simprior.temp.requires_grad_(False)
