...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0018 switch (non-module flag: no RNG consumption).
        self.use_lingain = _get(cfg, "use_lingain", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        if self.use_simprior:
            ...
            elif self.use_lingain:
                if self.use_peakcal:
                    cond_map = cond_map + self.simprior(fine, e, use_lingain=True, w_lin=self.lingain.w_lin, use_peakcal=True, w_p=self.peakcal.w_p)
                else:
                    cond_map = cond_map + self.simprior(fine, e, use_lingain=True, w_lin=self.lingain.w_lin)
            else:
                if self.use_peakcal:
...
class SimPrior(nn.Module):
    ...
    def forward(self, fine, e, use_cellcal=False, w_c=None, use_peakcal=False, w_p=None, use_lingain=False, w_lin=None):
        ...
        if use_cellcal:
            ...
            gain = (1 + w_lin * mhat).clamp_min(0.0)                   # (B,1,96,96); w_lin=0 -> all ones
            ev = ev * gain                                             # broadcast over the 2 evidence channels
        if use_peakcal:
            m = fine.detach().mean(dim=1, keepdim=True).clamp_min(0)   # (B,1,96,96)
            mbar = m.mean(dim=(2, 3), keepdim=True)                    # (B,1,1,1)
...
class LinGain(nn.Module):
    """H0018 linear wide-swing per-cell gain (`use_lingain`).

    Scalar-only pluggable module: ONE zero-init scalar `w_lin`. The gain math
    itself lives in `SimPrior.forward` (gated on the head flag
    `use_lingain` as an ELIF alternative to the cellcal block); this module
    only owns `w_lin` so the head flag stays a non-module switch like
    `use_simprior`/`use_cellcal`/`use_peakcal`. Zero-init draws no RNG, so
    append-only construction keeps parent init draws identical. NO
    normalization layers (train/eval agree exactly).
    """
    def __init__(self):
        super().__init__()
        self.w_lin = nn.Parameter(torch.zeros(()))
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0018: LinGain scalar is constructed LAST, after the PeakCal attach
        # (append-only RNG order, AGENTS rule 13). Zero-init draws no RNG, so
        # parent init draws keep their exact RNG positions. Forward gating
        # lives in CountingHead.forward via self.use_lingain (elif alternative
        # to the cellcal block in SimPrior.forward).
        self.head.lingain = LinGain()
        ...
        # Temp-pin hygiene: when use_lingain is on, pin simprior temp
        # (W line already detaches — value-identical detach in forward closes
        # the free-temp path; the linear gain must not reopen it).
        if self.head.use_lingain:
            self.head.simprior.temp.requires_grad_(False)
