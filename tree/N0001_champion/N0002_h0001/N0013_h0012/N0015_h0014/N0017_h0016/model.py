...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0016 switch (non-module flag: no RNG consumption).
        self.use_enmass = _get(cfg, "use_enmass", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        # H0016 (use_enmass): additive energy-mass residual into cond.
        ...
        if self.use_enmass:
            ...
            cond_map = cond_map + self.enmass(mhat)                    # (B,64,96,96) pre-decoder residual
...
class EnMass(nn.Module):
    """H0016 additive energy-mass residual (`use_enmass`).

    ONE zero-init 1x1 Conv2d(1, 64, 1): weight AND bias zero-init (+128
    params). Forward(mhat) returns proj(mhat) (B,64,96,96). Similarity-
    independent: no match/prototypes/queries/attention/temperature, NO norm
    layers (train/eval agree exactly). Zero-init draws no RNG, so
    append-only construction keeps parent init draws identical.
    """
    def __init__(self, out_dim=64):
        super().__init__()
        self.proj = nn.Conv2d(1, out_dim, 1)
        nn.init.zeros_(self.proj.weight)
        nn.init.zeros_(self.proj.bias)

    def forward(self, mhat):
        return self.proj(mhat)                                  # (B,64,96,96), all-zeros at init
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0016: EnMass projection is constructed LAST, after the PeakCal attach
        # (append-only RNG order, AGENTS rule 13). Zero-init draws no RNG, so
        # parent init draws keep their exact RNG positions. Forward gating
        # lives in CountingHead.forward via self.use_enmass.
        self.head.enmass = EnMass(out_dim=_get(cfg, "cond_dim", 64))
        # Temp-pin hygiene: when use_enmass is on, pin simprior temp
        # (value-identical detach in forward closes the free-temp path).
        if self.head.use_enmass:
            self.head.simprior.temp.requires_grad_(False)
