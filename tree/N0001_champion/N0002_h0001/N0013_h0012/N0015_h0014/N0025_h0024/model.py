...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0024 switch (non-module flag: no RNG consumption).
        self.use_canchor = _get(cfg, "use_canchor", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        if self.use_simprior:
            if self.use_cellcal:
                if self.use_peakcal:
                    cond_map = cond_map + self.simprior(fine, e, use_cellcal=True, w_c=self.cellcal.w_c, use_peakcal=True, w_p=self.peakcal.w_p, bboxes=bboxes_in, img_size=self.S, use_canchor=self.use_canchor)
                else:
                    cond_map = cond_map + self.simprior(fine, e, use_cellcal=True, w_c=self.cellcal.w_c, bboxes=bboxes_in, img_size=self.S, use_canchor=self.use_canchor)
            else:
                if self.use_peakcal:
                    cond_map = cond_map + self.simprior(fine, e, use_peakcal=True, w_p=self.peakcal.w_p, bboxes=bboxes_in, img_size=self.S, use_canchor=self.use_canchor)
                else:
                    cond_map = cond_map + self.simprior(fine, e, bboxes=bboxes_in, img_size=self.S, use_canchor=self.use_canchor)
...
class SimPrior(nn.Module):
    ...
    def forward(self, fine, e, use_cellcal=False, w_c=None, use_peakcal=False, w_p=None,
                bboxes=None, img_size=None, use_canchor=False):
        ...
        if use_canchor and bboxes is not None:
            ...
            idx = torch.arange(B, device=bboxes.device, dtype=bboxes.dtype).view(-1, 1, 1).expand(-1, K, 1)
            ...
            return self.out(ev) * alpha                          # scale the RESIDUAL, not ev, not gain
