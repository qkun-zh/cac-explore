...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.use_margin = _get(cfg, "use_margin", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        if self.use_simprior:
            if self.use_margin:
                cond_map = cond_map + self.simprior(fine, e, use_margin=True, w_m=self.margincal.w_m)
...
class SimPrior(nn.Module):
    ...
    def forward(self, fine, e, use_cellcal=False, w_c=None, use_peakcal=False, w_p=None, use_margin=False, w_m=None):
        ...
        if use_margin:
            ...
            gain = torch.exp((w_m * torch.log1p(mhat)).clamp(-3, 3))
            pos = (ev > 0).float().detach()
            ev = torch.where(pos > 0.5, ev * gain, ev)
            return self.out(ev)
        if use_cellcal:
            m = fine.detach().mean(dim=1, keepdim=True).clamp_min(0)   # (B,1,96,96)
            mbar = m.mean(dim=(2, 3), keepdim=True)                    # (B,1,1,1)
...
class MarginCal(nn.Module):
    """H0022 margin-sign-selective per-cell energy gain (`use_margin`).

    Scalar-only pluggable module: ONE zero-init scalar `w_m`. The gain math
    itself lives in `SimPrior.forward` (gated on the head flag
    `use_margin`); this module only owns `w_m` so the head flag stays a
    non-module switch like `use_simprior`. Zero-init draws no RNG, so
    append-only construction keeps parent init draws identical.
    """
    def __init__(self):
        super().__init__()
        self.w_m = nn.Parameter(torch.zeros(()))
