...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0015 switch (non-module flag: no RNG consumption).
        self.use_dualcal = _get(cfg, "use_dualcal", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        if self.use_simprior:
            if self.use_cellcal:
                if self.use_peakcal:
                    cond_map = cond_map + self.simprior(fine, e, use_cellcal=True, w_c=self.cellcal.w_c, use_peakcal=True, w_p=self.peakcal.w_p, use_dualcal=self.use_dualcal, w_hi=self.dualcal.w_hi, w_lo=self.dualcal.w_lo)
                else:
                    cond_map = cond_map + self.simprior(fine, e, use_cellcal=True, w_c=self.cellcal.w_c, use_dualcal=self.use_dualcal, w_hi=self.dualcal.w_hi, w_lo=self.dualcal.w_lo)
            else:
                if self.use_peakcal:
                    cond_map = cond_map + self.simprior(fine, e, use_peakcal=True, w_p=self.peakcal.w_p, use_dualcal=self.use_dualcal, w_hi=self.dualcal.w_hi, w_lo=self.dualcal.w_lo)
                else:
                    cond_map = cond_map + self.simprior(fine, e, use_dualcal=self.use_dualcal, w_hi=self.dualcal.w_hi, w_lo=self.dualcal.w_lo)
...
class SimPrior(nn.Module):
    ...
    def forward(self, fine, e, use_cellcal=False, w_c=None, use_peakcal=False, w_p=None, use_dualcal=False, w_hi=None, w_lo=None):
        ...
        if use_dualcal:
            ...
            w = torch.where((nelev >= 1100).view(-1, 1, 1, 1), w_hi, w_lo)  # per-image regime pick, detached routing
...
class DualCal(nn.Module):
    """H0015 two-regime per-cell gains routed by fixed spatial-extent cut (`use_dualcal`).

    Scalar-only pluggable module: TWO zero-init scalars `w_hi`, `w_lo`. The
    gain math itself lives in `SimPrior.forward` (gated on the head flag
    `use_dualcal`); this module only owns the weights so the head flag stays
    a non-module switch like `use_simprior`/`use_cellcal`/`use_peakcal`.
    Zero-init draws no RNG, so append-only construction keeps parent init
    draws identical. NO normalization layers (train/eval agree exactly).
    """
    def __init__(self):
        super().__init__()
        self.w_hi = nn.Parameter(torch.zeros(()))
        self.w_lo = nn.Parameter(torch.zeros(()))
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0015: DualCal scalars are constructed LAST, after the PeakCal attach
        # (append-only RNG order, AGENTS rule 13). Zero-init draws no RNG, so
        # parent init draws keep their exact RNG positions. Forward gating
        # lives in CountingHead.forward via self.use_dualcal.
        self.head.dualcal = DualCal()
        # Temp-pin hygiene: when use_dualcal is on, pin simprior temp
        # (value-identical detach in forward closes the free-temp path).
        if self.head.use_dualcal:
            self.head.simprior.temp.requires_grad_(False)
