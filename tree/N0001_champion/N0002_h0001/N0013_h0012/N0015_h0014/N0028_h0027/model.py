...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0027 switch (non-module flag: no RNG consumption).
        self.use_finescale = _get(cfg, "use_finescale", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        # H0027 (use_finescale): geometry capacity scale on the decoder-bound
        # fine channels ONLY, at the single pre-decoder cat site. cond_map,
        # SimPrior, the Condenser and the GCA bias path all keep reading the
        # unscaled `fine`; nothing after decoder(...) is touched. Flag off ->
        # dec_fine is `fine` itself, so the executed line is the parent's.
        dec_fine = fine
        if self.use_finescale and getattr(self, "finescale", None) is not None:
            fscale = self.finescale(bboxes_in, self.S)
            dec_fine = fine * fscale.view(-1, 1, 1, 1)
        dens = self.decoder(torch.cat([dec_fine, cond_map], 1))
        e_mean = e.mean(dim=1)
...
class FineScale(nn.Module):
    """H0027 geometry capacity scale on the decoder-bound fine channels:
    zero-init w_f -> f = clamp((me/32)^w_f, 0.25, 4).
    ME = mean_K(S^2 / (w_k * h_k)) from annotation boxes (CACViT geometry).
    torch.zeros(()) draws no RNG; step-0 w_f=0 => f=1 exact.
    """
    def __init__(self):
        super().__init__()
        self.w_f = nn.Parameter(torch.zeros(()))
    ...
    def forward(self, bboxes, img_size):
        ...
        f = torch.exp(self.w_f * (torch.log(me) - math.log(32.0))).clamp(0.25, 4.0)
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0027: FineScale scalar is constructed AFTER every parent module
        # (append-only RNG order, AGENTS rule 13). torch.zeros(()) draws no
        # RNG, so parent init draws keep their exact RNG positions. Forward
        # gating lives in CountingHead.forward via self.use_finescale.
        if self.head.use_finescale:
            self.head.finescale = FineScale()
