...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0004 switch (non-module flag: no RNG consumption, safe before SimBank wiring).
        self.use_simbank = _get(cfg, "use_simbank", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        # H0004 (use_simbank): additive finer 1/8 exemplar-token similarity bank
        # residual into cond. Flag off -> line skipped and forward is the parent's.
        if self.use_simbank:
...
class SimBank(nn.Module):
    ...
    def forward(self, h2, bboxes, fine, img_size):
        ...
        p = F.normalize(p, dim=-1).reshape(B, K * self.r * self.r, -1)  # (B, K*J, 64)
        q = F.normalize(self.qproj(fine), dim=1)                   # (B, 64, 96, 96)
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0004: SimBank is constructed AFTER all parent modules and after
        # SimPrior (append-only RNG order, AGENTS rule 13), so every parent and
        # SimPrior init draw keeps its exact RNG position; only SimBank's own
        # qproj/bank_proj init draws are appended. Forward gating lives in
        # CountingHead.forward via self.use_simbank.
