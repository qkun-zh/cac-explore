...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0009 switch (non-module flag: no RNG consumption).
        self.use_densexp = _get(cfg, "use_densexp", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        # H0009 (use_densexp): post-decoder dense-tail expert residual on density logits.
        # Flag off -> skipped, exact parent forward. Flag on at init -> g*R = 0 (zero-init out).
        if self.use_densexp:
            dens = dens + self.densexp(cond_map)
        e_mean = e.mean(dim=1)
...
class DenseTailExpert(nn.Module):
    """H0009 dense-tail expert residual (`use_densexp`).

    Post-decoder additive expert on density logits, gated per image by a
    stop-gradient dense-level proxy. Own params only (~4.3k); no S recompute,
    no shared projection, no qproj/kproj contact, no temperature touch, no
    global affine/count rescale, no dropout. Decoder in_ch stays 192.
    Zero-init `out` makes step-0 forward numerically identical to the parent.
    """
    def __init__(self, cond_dim=64):
        super().__init__()
        self.exp1 = nn.Conv2d(cond_dim, 32, 1)   # 64 -> 32, own weights
        self.exp2 = nn.Conv2d(32, cond_dim, 1)   # 32 -> 64, own weights
        self.out = nn.Conv2d(cond_dim, 1, 1)     # 64 -> 1, ZERO-INIT
        nn.init.zeros_(self.out.weight)
        nn.init.zeros_(self.out.bias)
        self.gate = nn.Linear(cond_dim, 1)       # detached GAP(64) -> scalar, sigmoid to [0,1]
        nn.init.zeros_(self.gate.weight)
        nn.init.zeros_(self.gate.bias)

    def forward(self, cond_map):
        # cond_map: (B,64,Hf,Wf). Proxy is stop-gradient; expert trunk keeps grad.
        proxy = cond_map.detach().mean(dim=(2, 3))              # (B,64) per-image, no grad into base
        g = torch.sigmoid(self.gate(proxy)).view(-1, 1, 1, 1)   # (B,1,1,1) in [0,1]
        h = F.gelu(self.exp1(cond_map))
        h = F.gelu(self.exp2(h))
        r = self.out(h)                                         # (B,1,Hf,Wf), all-zeros at init
        return g * r
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0009: DenseTailExpert is constructed AFTER every parent module
        # (append-only RNG order, AGENTS rule 13) and attached to the head, so
        # all parent init draws keep their exact RNG positions; only the
        # expert's own draws are appended. Forward gating lives in
        # CountingHead.forward via self.use_densexp.
        self.head.densexp = DenseTailExpert(cond_dim=_get(cfg, "cond_dim", 64))
