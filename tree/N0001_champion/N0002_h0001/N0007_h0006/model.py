...
class CountingHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # H0006 switch (non-module flag: no RNG consumption).
        self.use_h2pool = _get(cfg, "use_h2pool", False)
    ...
    def forward(self, h2, h3, bboxes_in):
        ...
        # H0006 (use_h2pool): additive h2-pooled 3-key bank residual into cond_map
        # at 48x48, upsampled to 96x96. Flag off -> skipped, forward is parent's byte-for-byte.
        if self.use_h2pool:
...
class H2Pool(nn.Module):
    """H0006 per-exemplar h2 attention-pooled 3-key bank (`use_h2pool`).

    Reads frozen h2 (1/8, 192ch) read-only via 3x3 ROI-align + attention-pool
    into K=3 keys; simprior-style K-normalized readout at 48x48 (shared
    simprior.qproj on the query side, own kproj 192->64 on the key side),
    zero-init 1x1 out added residually into cond_map (upsampled to 96x96).
    Decoder in_ch stays 192; no parent module shape/init touched.
    """
...
class Counter(nn.Module):
    def __init__(self, cfg):
        ...
        # H0006: H2Pool is constructed AFTER SimPrior (append-only RNG order,
        # AGENTS rule 13) and attached to the head as the LAST nn.Module in
        # __init__, so all parent + SimPrior init draws keep their exact RNG
        # positions; only H2Pool's own pool_attn/kproj init draws are appended.
        # Forward gating lives in CountingHead.forward via self.use_h2pool.
        self.head.h2pool = H2Pool(d_h2=dims[0], d_proj=64,
                                  cond_dim=_get(cfg, "cond_dim", 64))
