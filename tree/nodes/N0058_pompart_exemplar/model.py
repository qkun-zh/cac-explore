"""N0058_pompart_exemplar — frozen ConvNeXt Tiny + pluggable CountingHead + Part-wise Polynomial-Moment Exemplar (PMOM).
Pluggable base = N0054, with use_ddca=False (DDCA harmful, +1.8) and use_gca=True (the winning aux).
N0058 REPLACES the per-exemplar aggregation operator (2-layer self-attention + attention-pool over 49 ROI
patch tokens) with a part-wise 2nd-order polynomial-moment summary (PMOM). Each exemplar ROI is pooled
into 2x2 parts, gated sum of mean+square moment vectors projected to d_model, and additively fused into
its exemplar token ABOVE the XScale coarse summary and shape token. Keeps the same single fused-prototype
interface the load-bearing Condenser consumes (the allowed exemplar-embedding interface, §5.14), ~0.2M
params, single-switch use_pmom. use_pmom=False restores the exact N0054 attention path.
"""
...
class ExemplarEncoder(nn.Module):
    def __init__(self,in_dim=384,d_model=256,n_layers=2,n_heads=4,roi_size=7,use_xscale=False,xs=3,use_pmom=False):
        super().__init__()
        self.r=roi_size
        ...
        if use_pmom:
            self.gate=nn.Sequential(nn.Linear(2*in_dim,16), nn.GELU(), nn.Linear(16,1))
            self.moment_proj=nn.Linear(2*in_dim,d_model)
        else:
    ...
    def forward(self,feat,bboxes,img_size):
        ...
        if self.use_pmom:
            parts=F.adaptive_avg_pool2d(roi,2)
            h=parts.flatten(2).transpose(1,2)                  # (B*K,4,in_dim)
...
class CountingHead(nn.Module):
    ...
    def __init__(self,cfg):
        ...
        self.exemplar=ExemplarEncoder(in_dim=dims[1],d_model=_get(cfg,"embed_dim",256),n_layers=_get(cfg,"exemplar_layers",2),roi_size=_get(cfg,"roi_size",7),use_xscale=_get(cfg,"use_xscale",False),xs=_get(cfg,"xscale_size",3),use_pmom=_get(cfg,"use_pmom",False))
