"""N0038_rl_psf — frozen ConvNeXt Tiny + FineFuser + RL 5x5 PSF."""
...
class RLPSF(nn.Module):
    def __init__(self, fine_dim=128):
        super().__init__()
        self.params=nn.Parameter(torch.zeros(25))
        self.gain=nn.Sequential(nn.Conv2d(fine_dim,16,3,padding=1), nn.GroupNorm(4,16), nn.GELU(), nn.Conv2d(16,1,1))
        nn.init.zeros_(self.gain[-1].weight); nn.init.zeros_(self.gain[-1].bias)
    def forward(self, dens0, fine):
        k=F.softmax(self.params, dim=0).view(1,1,5,5)
        # flux conserving blur then gain
        dens_blur=F.conv2d(dens0, k, padding=2)
        g=torch.tanh(self.gain(fine))
        dens=dens_blur * (1 + 0.2 * g)
        return F.softplus(dens)
class Counter(nn.Module):
    ...
    def forward(self,imgs,bboxes,bboxes3=None):
        ...
        dens0=self.decoder(torch.cat([fine,cond.transpose(1,2).reshape(B,-1,Hf,Wf)],1))
        dens=self.rl(dens0, fine)
        return {"density":dens}
