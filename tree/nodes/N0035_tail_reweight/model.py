"""N0035_refine_frozen — frozen ConvNeXt Tiny + FineFuser + residual density refine (structural, optimizer unchanged)."""
...
class RefineBlock(nn.Module):
    def __init__(self, in_ch=129):
        super().__init__()
        self.net=nn.Sequential(nn.Conv2d(in_ch,64,3,padding=1), nn.GroupNorm(8,64), nn.GELU(),
                               nn.Conv2d(64,32,3,padding=1), nn.GroupNorm(8,32), nn.GELU(),
                               nn.Conv2d(32,1,1))
        nn.init.zeros_(self.net[-1].weight); nn.init.zeros_(self.net[-1].bias)
    def forward(self, fine, dens0):
        x=torch.cat([fine, dens0],1)
        return self.net(x)
class Counter(nn.Module):
    def __init__(self,cfg):
        ...
        self.decoder0=DensityDecoder(in_ch=D+_get(cfg,"cond_dim",64),hidden=2*D)
        self.refine=RefineBlock(in_ch=D+1)
    def train(self,mode=True):
        super().train(mode)
        self.backbone.eval()
        return self
    def forward(self,imgs,bboxes,bboxes3=None):
        ...
        dens0=self.decoder0(torch.cat([fine,cond.transpose(1,2).reshape(B,-1,Hf,Wf)],1))
        delta=self.refine(fine, dens0)
        dens=F.softplus(dens0 + delta)
        return {"density":dens}
