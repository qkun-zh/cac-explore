"""N0045_iter_exemplar — N0036 GCA+DDCA + iterative exemplar refinement."""
...
class IterRefine(nn.Module):
    def __init__(self, d_model=256, n_heads=4):
        ...
        # zero-init final linear
        nn.init.zeros_(self.ffn[-1].weight); nn.init.zeros_(self.ffn[-1].bias)
    def forward(self, e, fine_tokens):
        # e: [B,K,d], fine: [B,L,d]
        a,_=self.attn(self.norm1(e), fine_tokens, fine_tokens, need_weights=False)
        e2=self.norm1(e + a*0.1)
        e2=self.norm2(e2 + self.ffn(e2)*0.1)
        return e2
...
class Counter(nn.Module):
    def __init__(self,cfg):
        ...
        self.proj_fine=nn.Linear(D, d_model)
        self.iter1=IterRefine(d_model,4)
        self.iter2=IterRefine(d_model,4)
    ...
    def forward(self,imgs,bboxes,bboxes3=None):
        ...
        # iterative refinement with fine tokens
        fine_tok=self.proj_fine(fine.permute(0,2,3,1).reshape(B, Hf*Wf, -1))
        e=self.iter1(e, fine_tok)
        e=self.iter2(e, fine_tok)
