"""N0057_cond_matcher — frozen ConvNeXt Tiny + pluggable CountingHead + Cosine-Similarity Condenser (REPLACEMENT).
Base = N0054 (use_gca=True, use_ddca=False, use_xscale=True -> 19.647). Contrast test of the ONE untested
structural class: not another stacked add-on (all add-ons degrade), but a REPLACEMENT of the condenser.
All prior losers ADDED capacity (gates/keys/extra-scales). N0057 REPLACES the learned cross-attn
Condenser with a cosine-similarity prototype matcher (no MultiheadAttention, no FFN, no LayerNorms) —
far fewer params, directly testing whether the learned attention condenser pays for itself under frozen
30ep ('less is more'). Single-switch use_matcher within Condenser; identical surrounding head.
"""
...
class Condenser(nn.Module):
    """Cross-attn condenser (N0054 base). OR, when use_matcher=True, a REPLACEMENT cosine-similarity
    prototype matcher (no learned attention, no FFN): tests 'less is more' — whether the learned
    cross-attn condenser pays for its capacity under frozen 30ep. Single-switch replacement."""
    def __init__(self,d_in=128,d_sim=256,n_heads=4,ff=512,d_out=64,use_matcher=False):
        super().__init__()
        self.use_matcher=use_matcher
        self.proj_in=nn.Linear(d_in,d_sim)
        self.out=nn.Linear(d_sim,d_out)
        if use_matcher:
            # cosine-similarity matcher: normalize query/keys, weight exemplar projections per query
            return
    ...
    def forward(self,tok,e):
        if self.use_matcher:
            q=self.proj_in(tok)          # (B,N,d_sim)
            k=F.normalize(e,dim=-1)      # (B,K,d_sim) exemplar keys
            qn=F.normalize(q,dim=-1)
            w=torch.softmax(qn@k.transpose(1,2)/0.1,dim=-1)  # (B,N,K)
            agg=w@e                      # (B,N,d_sim) weighted exemplar summary
            return self.out(agg)
...
class CountingHead(nn.Module):
    ...
    def __init__(self,cfg):
        ...
        self.exemplar=ExemplarEncoder(in_dim=dims[1],d_model=_get(cfg,"embed_dim",256),n_layers=_get(cfg,"exemplar_layers",2),roi_size=_get(cfg,"roi_size",7),use_xscale=_get(cfg,"use_xscale",False),xs=_get(cfg,"xscale_size",3))
        self.cond=Condenser(d_in=D,d_sim=_get(cfg,"embed_dim",256),d_out=_get(cfg,"cond_dim",64),use_matcher=_get(cfg,"use_matcher",False))
