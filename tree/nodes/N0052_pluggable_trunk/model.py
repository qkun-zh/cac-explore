"""N0052_pluggable_trunk — frozen ConvNeXt Tiny + single pluggable CountingHead trunk.
Behavior-identical to N0036 (MAE ~20.49 target). Refactor ONLY for pluggability (§5.14):
  - Trunk (FineFuser + ExemplarEncoder + Condenser + DensityDecoder) = ONE module (CountingHead).
  - GCA and DDCA = independent, single-switch components gated by config flags (use_gca / use_ddca).
  - No component reads another's internals; GCA reads only GAP(fine) + e_mean (shared interfaces).
"""
