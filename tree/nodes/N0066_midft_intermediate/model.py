"""N0066_midft_intermediate — GCA+XScale on intermediate readout with mid-stage FT.
Backbone: DINOv3-ConvNeXt-Tiny hs_map (2,3) dims (192,384), stages 1,2 FT @0.1x LR.
Head identical to N0054; only delta is backbone tune_stages + param_groups + train().
"""
...
class Backbone(nn.Module):
    def __init__(self,cfg):
        ...
        self.hs_map=tuple(_get(cfg,"hs_map",(2,3)))
        self.tune_stages=list(_get(cfg,"tune_stages",[1,2]))
        self.backbone_lr=float(_get(cfg,"backbone_lr",1e-4))
        # freeze all, then unfreeze selected stages
        ...
        for idx in self.tune_stages:
            try:
                ...
                pass
        # also unfreeze corresponding downsample if stage FT (needed for stage input adaptation)
        # keep stage0 and stage3 frozen when not in tune list
        dims_map={1:96,2:192,3:384,4:768}
        dims=tuple(dims_map[i] for i in self.hs_map)
        self.out_channels=list(dims)
        self._dims=dims
    @torch.no_grad()
    def forward_feature_map_frozen(self,x):
    ...
    def forward_feature_map(self,x):
        # when any stage is tuned, need grad; otherwise no_grad is ok but we keep grad path
        hs=self.net(pixel_values=x, output_hidden_states=True).hidden_states
        return [hs[i] for i in self.hs_map]
    def train(self,mode=True):
        super().train(mode)
        # keep frozen stages in eval, tuned stages in train
        try:
            self.net.train(mode)
            for i, stage in enumerate(self.net.model.stages):
                if i in self.tune_stages:
                    ...
                    stage.eval()
            # keep embeddings/norm in eval if not tuned
            if 0 not in self.tune_stages:
                # stage0 frozen -> keep its downsample in eval to avoid BN drift (though LN)
                pass
        except Exception:
...
class ExemplarEncoder(nn.Module):
    def __init__(self,in_dim=384,d_model=256,n_layers=2,n_heads=4,roi_size=7,use_xscale=True,xs=3):
        super().__init__()
        self.r=roi_size
...
class Counter(nn.Module):
    ...
    def param_groups(self, lr, weight_decay):
        # differential LR: backbone tuned stages at 0.1x, head at 1x
    ...
    def train(self,mode=True):
        super().train(mode)
        # delegate backbone train/eval handling (keeps frozen stages in eval)
