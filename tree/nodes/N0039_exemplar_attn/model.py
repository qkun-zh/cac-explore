"""N0039_exemplar_attn — N0036 GCA+DDCA + exemplar spatial attention."""
...
class Counter(nn.Module):
    ...
    def forward(self,imgs,bboxes,bboxes3=None):
        ...
        B=imgs.shape[0]
        Hf=Wf=self.S//4
        fmap=fine.permute(0,2,3,1).flatten(1,2)  # [B, Hf*Wf, D]
        e=self.exemplar(h3,bboxes_in,self.S)     # [B, K, d_model]
        ...
        dens=dens+bias
        # Exemplar spatial attention
        q=self.e2d_q(e.mean(dim=1))             # [B, D//4]
        k=self.e2d_k(fine)                       # [B, D//4, Hf, Wf]
        attn=torch.einsum('bd,bdhw->bhw',q,k)   # [B, Hf, Wf]
        attn=attn*0.1*self.e2d_scale            # small init
        dens=dens * (1+torch.sigmoid(attn).unsqueeze(1))
