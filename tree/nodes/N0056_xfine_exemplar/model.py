"""N0056_xfine_exemplar — frozen ConvNeXt Tiny + pluggable CountingHead + Multi-Scale Exemplar (XScale+XFine).
Base = N0054 (use_gca=True, use_ddca=False, use_xscale=True -> 19.647). N0055 showed that SEPARATE 2nd
condenser keys (2K keys) degrade (+1.19): the single fused prototype is a better condenser input.
N0056 therefore KEEPS N0054's exact fused-single-prototype XScale mechanism and adds XFine: a higher-
resolution (h2, 1/8, 192ch) exemplar ROI pooled->GAP->proj, fused ADDITIVELY into the same exemplar
token (same fusion recipe as the winning XScale). Adds a sharper, scale-richer exemplar view without
changing key structure (avoids N0055's dilution). ~0.1M params, single-switch use_xfine.
"""
...
class ExemplarEncoder(nn.Module):
    def __init__(self,in_dim=384,d_model=256,n_layers=2,n_heads=4,roi_size=7,use_xscale=False,xs=3,use_xfine=False,d_fine_enc=None):
        super().__init__()
        self.r=roi_size
        self.use_xscale=use_xscale
        self.use_xfine=use_xfine
        ...
        if use_xfine:
            # fine-scale exemplar summary from h2 (1/8): higher-res ROI -> GAP -> proj -> add (N0054-style fused)
            self.fproj=nn.Linear(d_fine_enc,d_model)
    def forward(self,feat,bboxes,img_size,feat_fine=None):
        B,C,H,W=feat.shape
        K=bboxes.shape[1]
        s=W/float(img_size)
        ...
        if self.use_xfine and feat_fine is not None:
            # fine-scale summary from h2 (1/8): boxes re-scaled by h2 width; higher-res ROI -> GAP -> proj -> add
            _,_,Hf2,Wf2=feat_fine.shape
            sf=Wf2/float(img_size)
            rois_f=torch.cat([idx,bboxes*sf],-1).reshape(B*K,5)
            roi_f=roi_align(feat_fine,rois_f,output_size=(self.r,self.r))
            fine_sum=F.adaptive_avg_pool2d(roi_f,(1,1)).squeeze(-1).squeeze(-1)  # (B*K,d_fine_enc)
            out=out+self.fproj(fine_sum).view(B,K,-1)
        return out
...
class CountingHead(nn.Module):
    ...
    def __init__(self,cfg):
        ...
        self.exemplar=ExemplarEncoder(in_dim=dims[1],d_model=_get(cfg,"embed_dim",256),n_layers=_get(cfg,"exemplar_layers",2),roi_size=_get(cfg,"roi_size",7),use_xscale=_get(cfg,"use_xscale",False),xs=_get(cfg,"xscale_size",3),use_xfine=_get(cfg,"use_xfine",False),d_fine_enc=dims[0])
        ...
        self.use_xfine=_get(cfg,"use_xfine",False)
    def forward(self,h2,h3,bboxes_in):
        B=bboxes_in.shape[0]
        fine=self.fuser(h2,h3)
        Hf=Wf=self.S//4
        fmap=fine.permute(0,2,3,1).flatten(1,2)
        e=self.exemplar(h3,bboxes_in,self.S,feat_fine=(h2 if self.use_xfine else None))
