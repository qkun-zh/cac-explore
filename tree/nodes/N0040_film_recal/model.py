"""N0040_film_recal — N0036 GCA+DDCA + FILM recalibration of h2/h3 via exemplar."""
...
class Counter(nn.Module):
    def __init__(self,cfg):
        ...
        # FILM recalibration: e_mean -> gamma,beta for h2 (192ch) and h3 (384ch)
        self.film_h2=nn.Linear(d_model, dims[0]*2)  # 256 -> 384 (gamma+beta for 192ch)
        self.film_h3=nn.Linear(d_model, dims[1]*2)  # 256 -> 768 (gamma+beta for 384ch)
        nn.init.zeros_(self.film_h2.weight); nn.init.zeros_(self.film_h2.bias)
        nn.init.zeros_(self.film_h3.weight); nn.init.zeros_(self.film_h3.bias)
    def train(self,mode=True):
        super().train(mode)
        self.backbone.eval()
        return self
    def forward(self,imgs,bboxes,bboxes3=None):
        ...
        # FILM: exemplar-conditioned recalibration
        e=self.exemplar(h3,bboxes_in,self.S)
        e_mean=e.mean(dim=1)  # [B, d_model]
        # h2 FILM
        gb_h2=self.film_h2(e_mean)  # [B, 2*192]
        gamma_h2, beta_h2=gb_h2.chunk(2, dim=-1)
        gamma_h2=(1+gamma_h2.unsqueeze(-1).unsqueeze(-1))  # [B,192,1,1]
        beta_h2=beta_h2.unsqueeze(-1).unsqueeze(-1)  # [B,192,1,1]
        h2=h2*gamma_h2+beta_h2
        # h3 FILM
        gb_h3=self.film_h3(e_mean)
        gamma_h3, beta_h3=gb_h3.chunk(2, dim=-1)
        gamma_h3=(1+gamma_h3.unsqueeze(-1).unsqueeze(-1))
        beta_h3=beta_h3.unsqueeze(-1).unsqueeze(-1)
        h3=h3*gamma_h3+beta_h3
