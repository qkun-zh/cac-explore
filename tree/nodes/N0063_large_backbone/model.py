"""N0063_large_backbone — User-Guided backbone swap diagnostic.
Parent N0054 GCA+XScale head unchanged; backbone swapped from dinov3-convnext-tiny (27M)
to timm convnext_small.in12k (49.5M, same dims 192@1/8 + 384@1/16, deeper) to test if 32M cap is bottleneck.
Deviation: allows total ~53M >32M (max_params 60). use_large_backbone=False = exact champion.
"""
...
class Backbone(nn.Module):
    def __init__(self,cfg):
        super().__init__()
        self.use_large=_get(cfg,"use_large_backbone",False)
        dims=_get(cfg,"backbone_dims",(192,384))
        self.out_channels=list(dims)
        if self.use_large:
            import timm
            # convnext_small.in12k features [96,192,384,768] at strides 4,8,16,32; we need 1/8 + 1/16
            self.net=timm.create_model("convnext_small.in12k", pretrained=True, features_only=True, out_indices=(1,2))
            self.net.eval()
