"""N0061_countnorm — N0054 (GCA + XScale) + CountNormHead multiplier (use_countnorm).
CountNormHead bolts on parallel to GCA, reading ONLY shared interfaces GAP(fine) (B,128) +
e_mean (B,256). Computes a per-image log-scale z = clamp(MLP([GAP(fine)||e_mean]), -2, 2) and
multiplies the final density (post-GCA additive bias) by exp(z). W2 zero-init => z=0 => f=1
at ep0, so use_countnorm=True is identity-at-init (trajectory preserved) and use_countnorm=False
is bit-identical to N0054. Multiplicative, channel-invariant, spatial-free count autoscale.
"""
...
class CountNormHead(nn.Module):
    """Independent pluggable aux: per-image multiplicative count autoscale.
    Reads GAP(fine) + e_mean (shared interfaces, no coupling). z=clamp(MLP,[-2,2]); f=exp(z).
    W2 zero-init => z=0 => f=1 at init (identity-at-init). Multiplicative & channel-invariant.
    """
    def __init__(self,D,d_model):
        super().__init__()
        self.mlp=nn.Sequential(nn.Linear(D+d_model,64), nn.GELU(), nn.Linear(64,1))
        nn.init.zeros_(self.mlp[-1].weight)
        nn.init.zeros_(self.mlp[-1].bias)
    def forward(self,feat):
        return torch.clamp(self.mlp(feat), min=-2.0, max=2.0)
...
class Counter(nn.Module):
    def __init__(self,cfg):
        ...
        self.use_countnorm=_get(cfg,"use_countnorm",False)
        self.backbone=Backbone(cfg)
        self.head=CountingHead(cfg)
        if self.use_gca:
            self.gca=GCA(D,_get(cfg,"embed_dim",256))
        if self.use_countnorm:
            self.countnorm=CountNormHead(D,_get(cfg,"embed_dim",256))
    def train(self,mode=True):
        super().train(mode)
        self.backbone.eval()
        return self
    def forward(self,imgs,bboxes,bboxes3=None):
        ...
        if self.use_countnorm:
            dens=out["density"]
            gap=fine.mean(dim=(2,3))
            feat=torch.cat([gap,e_mean],dim=-1)
            z=self.countnorm(feat)
            f=z.exp()
            out["density"]=dens*f.view(-1,1,1,1)
        return out
