"""N0062_fine_decoder — frozen ConvNeXt Tiny + GCA + XScale + native-1/4 decoder injection.
Parent: N0054 (19.647). Adds single-switch `use_fine_decoder`: injects frozen native-1/4 h1 (96ch @ 1/4)
into DensityDecoder INPUT only (receiver-resolution enrichment). h1 is currently exposed to NOTHING;
fine (1/4) today is upsampled-from-1/8. Decoder in_ch 192->200 (+20k). No exemplar/condenser/GCA/attention/engine change.
use_fine_decoder=False = exact champion bit-identical.
"""
...
class Backbone(nn.Module):
    def __init__(self,cfg):
        ...
        self.hs_map=(2,3)
        self.hs_fine_idx=1
        self.use_fine_decoder=_get(cfg,"use_fine_decoder",False)
    @torch.no_grad()
    def forward_feature_map(self,x):
        hs=self.net(pixel_values=x, output_hidden_states=True).hidden_states
        out=[hs[i] for i in self.hs_map]
        if self.use_fine_decoder:
            out.append(hs[self.hs_fine_idx])
        return out
...
class CountingHead(nn.Module):
    ...
    use_fine_decoder: when True, injects native-1/4 h1 into decoder input via 1x1 96->8 + GN.
    """
    def __init__(self,cfg):
        super().__init__()
        ...
        extra=8 if self.use_fine_decoder else 0
        self.decoder=DensityDecoder(in_ch=D+_get(cfg,"cond_dim",64)+extra,hidden=2*D)
        self.S=_get(cfg,"input_size",384)
        if self.use_fine_decoder:
            self.injector=nn.Sequential(nn.Conv2d(96,8,1), nn.GroupNorm(2,8))
    def forward(self,h2,h3,bboxes_in,h1=None):
        B=bboxes_in.shape[0]
        ...
        cat_list=[fine,cond.transpose(1,2).reshape(B,-1,Hf,Wf)]
        if self.use_fine_decoder and h1 is not None:
            cat_list.append(self.injector(h1))
        dens=self.decoder(torch.cat(cat_list,1))
        e_mean=e.mean(dim=1)
...
class Counter(nn.Module):
    ...
    def forward(self,imgs,bboxes,bboxes3=None):
        ...
        h2,h3=feats[0],feats[1]
        h1=feats[2] if self.use_fine_decoder and len(feats)>2 else None
        dens,fine,e_mean=self.head(h2,h3,bboxes_in,h1=h1)
        B=imgs.shape[0]
        Hf=Wf=self.S//4
        out={"density":dens}
        if self.use_gca:
