"""N0036_gca_ddca — frozen ConvNeXt Tiny + FineFuser + GCA aux + DDCA (structural, optimizer unchanged)."""
...
class FineFuserDDCA(nn.Module):
    def __init__(self,ch_coarse,ch_mid,d_fine=128):
        ...
        # zero-init ctx for step0 identity
...
class Counter(nn.Module):
    ...
    def forward(self,imgs,bboxes,bboxes3=None):
        ...
        # GCA during training: compute aux count, but engine handles loss; we expose aux via dens? Instead we store aux in dict for engine to use if it checks, but engine will ignore.
        # We compute aux log count loss internally and return it as part of density? For now return density only; aux loss will be added via engine's w_cnt? Instead we compute aux and add to loss via returning extra key that engine doesn't use.
        # To make aux effective under frozen optimizer, we need to make it influence grads through a path that engine's loss will include.
        # Simplest: we add aux as an extra channel to density? No.
        # Instead, we will let engine compute its own loss, and GCA will be trained via auxiliary loss that we add to the density loss by making dens require extra grad from aux head via a dummy loss that engine will backprop if we return it.
        # Engine does: loss = mse(dens, gt) + w_cnt*l1(count). If we return dens, aux head won't be trained.
        # So we need to make aux head trainable via inference-time auxiliary: we will store gca output as attribute and hope engine's optimizer will still step it via density path? It won't.
        # Workaround: make gca influence dens via a tiny residual: dens = dens + 1e-3 * gca_broadcast -> then engine's loss will train gca.
        # Compute gca
        gap=fine.mean(dim=(2,3))  # [B,D]
        e_mean=e.mean(dim=1)  # [B,d_model]
        gca_in=torch.cat([gap, e_mean],1)
        n_aux=F.softplus(self.gca(gca_in)).squeeze(1)  # [B]
        # inject tiny influence so gca gets gradient from engine's count loss
        # broadcast n_aux to density as additive bias 0.02 * (n_aux / (Hf*Wf)) per cell -> sum bias = 0.02*n_aux (~2% of count) for gradient
        bias = (n_aux / float(Hf*Wf)).view(B,1,1,1) * 0.02
        dens = dens + bias
        # also store for debugging
        self._last_n_aux=n_aux
