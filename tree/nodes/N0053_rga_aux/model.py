"""N0053_rga_aux — frozen ConvNeXt Tiny + pluggable CountingHead trunk + Regionalized Count Auxiliary (RGA).
Pluggable base = N0052, with use_ddca=False per ablation (DDCA is harmful, +1.8).
N0053 adds RGA: an independent pluggable aux head that reads GAP(fine) + e_mean (shared interfaces),
predicts KxK regional log-counts, and injects a regional bias into density (like GCA's global bias)
so the engine's density MSE trains it regionally. Targets the RMSE-dominated tail.
"""
...
class RGA(nn.Module):
    """Independent pluggable aux: Regionalized Count Auxiliary. Reads GAP-per-region of fine + e_mean,
    predicts grid x grid regional log-counts, injects a regional bias into density so the engine's
    density MSE supervises regional structure (mirrors GCA's bias trick at region granularity).
    """
    def __init__(self,D,d_model,grid=4):
        super().__init__()
        self.grid=grid
        in_dim=D+d_model
        self.mlp=nn.Sequential(nn.Linear(in_dim,128), nn.GELU(), nn.Linear(128,1))
        nn.init.zeros_(self.mlp[-1].weight); nn.init.zeros_(self.mlp[-1].bias)
    def forward(self,fine,e_mean,Hf,Wf):
        g=self.grid
        B=fine.shape[0]
        feat=F.adaptive_avg_pool2d(fine,(g,g))     # B,D,g,g
        feat=feat.permute(0,2,3,1).reshape(B,g*g,-1) # B,g*g,D
        e_b=e_mean.unsqueeze(1).expand(B,g*g,-1)    # B,g*g,d_model
        n=F.softplus(self.mlp(torch.cat([feat,e_b],-1))).squeeze(-1)  # B,g*g
        region_area=float((Hf//g)*(Wf//g))
        bias=(0.02*n/region_area).reshape(B,1,g,g)   # B,1,g,g
        bias=F.interpolate(bias,size=(Hf,Wf),mode="nearest")
        return n, bias
...
class Counter(nn.Module):
    def __init__(self,cfg):
        ...
        self.use_rga=_get(cfg,"use_rga",False)
        self.rga_grid=int(_get(cfg,"rga_grid",4))
        self.backbone=Backbone(cfg)
        self.head=CountingHead(cfg)
        if self.use_gca:
            self.gca=GCA(D,_get(cfg,"embed_dim",256))
        if self.use_rga:
            self.rga=RGA(D,_get(cfg,"embed_dim",256),grid=self.rga_grid)
    def train(self,mode=True):
        super().train(mode)
        self.backbone.eval()
        return self
    def forward(self,imgs,bboxes,bboxes3=None):
        ...
        if self.use_gca:
            ...
            out["n_aux"]=n_aux
        if self.use_rga:
            n_rga,bias_rga=self.rga(fine,e_mean,Hf,Wf)
            out["density"]=out["density"]+bias_rga
            out["n_rga"]=n_rga
        return out
