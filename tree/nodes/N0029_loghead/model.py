"""N0029_loghead — champion recipe + log-space density head (GOD v4 seed 1).

Only delta vs N0027_norm_flip_swa: head outputs raw log-density, forward exps it.
"""
import math
import torch
...
class DinoLogHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.adapter = nn.Sequential(nn.Linear(ch, dim), nn.GELU(), nn.Dropout(drop),
                                     nn.Linear(dim, ch))
        # raw output = log-density; exp at the end
    ...
    def forward(self, imgs, bboxes):
        ...
        log_mass = self.head(adapted.transpose(1, 2).reshape(B, -1, ps, ps))
        mass = torch.exp(torch.clamp(log_mass, -10.0, 10.0))
        return {"density": mass}


def build_model(cfg):
    return DinoLogHead(cfg)
