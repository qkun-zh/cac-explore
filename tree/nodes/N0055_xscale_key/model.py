"""N0055_xscale_key — frozen ConvNeXt Tiny + pluggable CountingHead + XScale-Key (coarse exemplar KEY).
Pluggable base = N0054 (use_ddca=False, use_gca=True, use_xscale=True).
N0055 extends the winning exemplar-interface idea: instead of only FUSING the coarse ROI GAP into the
pooled fine exemplar vector (N0054 XScale), it also supplies the coarse prototype as a SEPARATE second
exemplar KEY to the Condenser (2K keys: fine-prototype + coarse-prototype per exemplar). This lets the
condenser weight fine-vs-coarse exemplar evidence per query, instead of receiving a pre-fused vector.
Purely exemplar-interface, no feature gating, no density bias, ~0.1M, single-switch use_xscale_key.
Control = N0054 (19.647).
"""
...
class ExemplarEncoder(nn.Module):
    def __init__(self,in_dim=384,d_model=256,n_layers=2,n_heads=4,roi_size=7,use_xscale=False,xs=3,use_xscale_key=False,xks=3):
        super().__init__()
        self.r=roi_size
        self.use_xscale=use_xscale
        self.use_xscale_key=use_xscale_key
        ...
        if use_xscale_key:
            self.xks=xks
            self.xk_proj=nn.Linear(in_dim,d_model)
    def forward(self,feat,bboxes,img_size):
        ...
        e_key=None
        if self.use_xscale:
            # coarse global summary per exemplar: pool at 2nd (smaller) scale -> GAP -> proj -> add (fuse)
        ...
        if self.use_xscale_key:
            # separate coarse prototype as an EXTRA exemplar key (not fused): (B,K,d_model)
            roi3=roi_align(feat,rois,output_size=(self.xks,self.xks))
            ck=F.adaptive_avg_pool2d(roi3,(1,1)).squeeze(-1).squeeze(-1)     # (B*K,in_dim)
            e_key=self.xk_proj(ck).view(B,K,-1)
        return out, e_key
...
class CountingHead(nn.Module):
    ...
    def __init__(self,cfg):
        ...
        self.exemplar=ExemplarEncoder(in_dim=dims[1],d_model=_get(cfg,"embed_dim",256),n_layers=_get(cfg,"exemplar_layers",2),roi_size=_get(cfg,"roi_size",7),use_xscale=_get(cfg,"use_xscale",False),xs=_get(cfg,"xscale_size",3),use_xscale_key=_get(cfg,"use_xscale_key",False),xks=_get(cfg,"xscale_key_size",3))
    ...
    def forward(self,h2,h3,bboxes_in):
        ...
        e,e_key=self.exemplar(h3,bboxes_in,self.S)
        keys=e if e_key is None else torch.cat([e,e_key],1)   # (B, K[ +K], d_model) condenser keys
        cond=self.cond(fmap,keys)
        dens=self.decoder(torch.cat([fine,cond.transpose(1,2).reshape(B,-1,Hf,Wf)],1))
        e_mean=e.mean(dim=1)  # shared interface for GCA (fine prototypes only)
