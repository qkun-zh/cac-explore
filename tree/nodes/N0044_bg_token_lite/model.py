"""N0044_bg_token_lite — N0036 GCA+DDCA + bg token in Condenser."""
...
class CondenserBg(nn.Module):
    def __init__(self,d_in=128,d_sim=256,n_heads=4,ff=512,d_out=64):
        ...
        self.bg_token=nn.Parameter(torch.randn(1,1,d_sim)*0.02)
    def forward(self,tok,e):
        tok=self.proj_in(tok)
        B,K,_=e.shape
        bg=self.bg_token.expand(B,1,-1)
        e_bg=torch.cat([e, bg], dim=1)
        a,_=self.attn(self.norm1(tok),e_bg,e_bg,need_weights=False)
        q=self.norm1(tok+a)
...
class Counter(nn.Module):
    def __init__(self,cfg):
        ...
        self.cond=CondenserBg(d_in=D,d_sim=d_model,d_out=_get(cfg,"cond_dim",64))
