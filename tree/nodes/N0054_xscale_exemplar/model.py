"""N0054_xscale_exemplar — frozen ConvNeXt Tiny + pluggable CountingHead + Multi-Scale Exemplar (XScale).
Pluggable base = N0052, with use_ddca=False (DDCA harmful, +1.8) and use_gca=True (the winning aux).
N0054 adds XScale: an exemplar-side refinement (NOT a density bias, NOT a feature modulator). Each
exemplar ROI is pooled at a second coarse scale (xscale_size), GAP'd to a per-exemplar global summary,
projected to d_model, and additively fused into its exemplar token BEFORE the Condenser. Enriches the
exemplar embedding the condenser consumes (the allowed exemplar-embedding interface, §5.14), ~0.1M
params, single-switch use_xscale. Targets cross-scale scale-invariance of exemplar appearance.
"""
