"""N0042_mutual_bg — N0036 GCA+DDCA + early mutual h3↔exemplar + bg token, zero-init."""
...
class MutualBg(nn.Module):
    def __init__(self, ch=384, d_model=256, n_heads=4):
        super().__init__()
        self.proj_q=nn.Linear(ch, d_model)
        self.proj_kv=nn.Linear(d_model, d_model)
        self.bg_token=nn.Parameter(torch.randn(1,1,d_model)*0.02)
        self.attn=nn.MultiheadAttention(d_model, n_heads, batch_first=True)
        self.out=nn.Linear(d_model, ch)
        self.ln1=nn.LayerNorm(d_model)
        self.ln2=nn.LayerNorm(ch)
        nn.init.zeros_(self.out.weight); nn.init.zeros_(self.out.bias)
    def forward(self, h3, e):
        # h3: [B,C,H,W], e: [B,K,d_model]
        B,C,H,W=h3.shape
        K=e.shape[1]
        # flatten h3
        q=self.proj_q(h3.permute(0,2,3,1).reshape(B, H*W, C))  # [B,HW,d]
        # kv = exemplar + bg token
        bg=self.bg_token.expand(B,1,-1)  # [B,1,d]
        kv=torch.cat([e, bg], dim=1)  # [B,K+1,d]
        kv=self.ln1(self.proj_kv(kv)) if False else self.ln1(kv)
        qn=self.ln1(q)
        # cross attn: q attends to kv
        out,_=self.attn(qn, kv, kv, need_weights=False)  # [B,HW,d]
        out=self.out(out)  # [B,HW,C]
        # zero-init residual
        h3_flat=h3.permute(0,2,3,1).reshape(B,H*W,C)
        h3_new=(h3_flat + out).permute(0,2,1).reshape(B,C,H,W)
        return h3_new
...
class Counter(nn.Module):
    def __init__(self,cfg):
        ...
        self.mutual=MutualBg(ch=dims[1], d_model=d_model, n_heads=4)
    ...
    def forward(self,imgs,bboxes,bboxes3=None):
        ...
        # initial exemplar
        e0=self.exemplar(h3,bboxes_in,self.S)
        # mutual refinement of h3
        h3=self.mutual(h3, e0)
        # re-encode exemplar from refined h3 (more target-aware)
