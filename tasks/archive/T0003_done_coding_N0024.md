...
- smoke evidence (2026-08-24, server /data/runs/smoke_N0024_ebc_partialft.log): `[model] N0024_ebc_partialft params total=24.24M trainable=5.73M (backbone trainable=3.55M @ lr_mult=0.1)`; `[engine] ... params=24.24M smoke=True` (<32M assert passed); E001 loss=2.0665 → E002+ loss=0.0000, no NaN/OOM; `RESULT ... "status": "success", "oom": false, "instability": false`; `SMOKE_EXIT=0`. Total params identical to parent N0022 train.log (24.24M) — freeze mask adds zero params (idea's ~23.2M was an estimate; DINOv2-S+reg4 base is larger). Trainable split: backbone 3.55M (blocks 10-11 + final norm) + head/adapter stack ~2.18M.
